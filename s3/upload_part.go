...
func (s *HTTP2Server) uploadPart(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	etag, data, err := model.CalculatePartETagFromReader(s.decodeBody(r))
	if err != nil {
		slog.ErrorContext(ctx, "Failed to read part data", "uploadID", uploadID, "part", partNumber, "error", err)
		s.handleError(w, r, model.NewS3Error(model.ErrInternalError, "Failed to read part data", 500))
		return
	}
	...
	if partSize > model.MaxPartSize {
		s.handleError(w, r, model.NewS3Error(model.ErrEntityTooLarge, "Part exceeds maximum size", 400))
		return
	}
	...
	if _, err := tmpFile.Write(data); err != nil {
		s.handleError(w, r, model.NewS3Error(model.ErrInternalError, "Failed to write temp file", 500))
		return
	}
	...
	if _, _, err := s.putObjectViaQUIC(ctx, tmpPath, objectHash); err != nil {
	...
	if err := gob.NewEncoder(&partBuf).Encode(partMeta); err != nil {
		s.handleError(w, r, model.NewS3Error(model.ErrInternalError, "Failed to encode part metadata", 500))
		return
	}
	if err := s.statePut(model.TableParts, multipartPartKey(uploadID, partNumber), partBuf.Bytes()); err != nil {
		slog.ErrorContext(ctx, "Failed to store part metadata", "uploadID", uploadID, "part", partNumber, "error", err)
		s.handleError(w, r, model.NewS3Error(model.ErrInternalError, "Failed to store part metadata", 500))
		return
	}

	placement, err := s.placeShards(objectHash, partSize)
	if err != nil {
	...
	if err := s.statePut(model.TableObjects, partShardKey(uploadID, partNumber), shardBuf.Bytes()); err != nil {
		slog.ErrorContext(ctx, "Failed to store part shard metadata", "uploadID", uploadID, "part", partNumber, "error", err)
		s.handleError(w, r, model.NewS3Error(model.ErrInternalError, "Failed to store part shard metadata", 500))
		return
	}
