...
func (s *HTTP2Server) getObject(w http.ResponseWriter, r *http.Request) {
	...
	shards, size, err := s.openInput(bucket, key)
	if err != nil {
	...
	if rangeStart >= 0 || rangeEnd >= 0 {
		body, contentRange, err = s.readRange(ctx, bucket, key, shards, size, rangeStart, rangeEnd)
		status = http.StatusPartialContent
	} else {
		body, err = s.readObject(ctx, bucket, key, shards, size)
	}
	if err != nil {
		s.handleError(w, r, err)
		return
...
func (s *HTTP2Server) readObject(ctx context.Context, bucket, key string, shards ObjectToShardNodes, size int64) ([]byte, error) {
	...
	shardReaders, err := s.shardReaders(objectHash, shards, false)
	if err != nil {
		return nil, model.NewS3Error(model.ErrInternalError, err.Error(), 500)
	}

	var out bytes.Buffer
	if err := enc.Join(&out, shardReaders, size); err != nil {
		slog.WarnContext(ctx, "Initial join failed, attempting reconstruction", "err", err)

		out.Reset()
		reconstructed, err := s.reconstructObject(ctx, objectHash, shards, enc, size)
		if err != nil {
...
func (s *HTTP2Server) readRange(ctx context.Context, bucket, key string, shards ObjectToShardNodes, totalSize, reqStart, reqEnd int64) (data []byte, contentRange string, err error) {
	...
	shardSize := (totalSize + int64(s.rsDataShard) - 1) / int64(s.rsDataShard)
	startShardIdx := min(int(start/shardSize), s.rsDataShard-1)
	endShardIdx := min(int(end/shardSize), s.rsDataShard-1)

	if startShardIdx == endShardIdx {
		objectHash := model.ObjectHash(bucket, key)
		data, err := s.readRangeFromSingleShard(ctx, objectHash, shards, startShardIdx, start, end, shardSize, totalSize)
		if err == nil {
	...
	full, err := s.readObject(ctx, bucket, key, shards, totalSize)
	if err != nil {
...
func (s *HTTP2Server) readRangeFromSingleShard(ctx context.Context, objectHash [32]byte, shards ObjectToShardNodes, shardIdx int, globalStart, globalEnd, shardSize, totalSize int64) ([]byte, error) {
	...
	reader, err := s.shards.GetShardRange(ctx, nodeNum, objectRequest)
	if err != nil {
