...
func stateGet(st Store, table, key string) ([]byte, error) {
	if st == nil {
...
func statePut(st Store, table, key string, value []byte) error {
	if st == nil {
...
func stateDelete(st Store, table, key string) error {
	if st == nil {
		return errNoStateClient
	}
...
func stateScan(st Store, table, prefix string, limit int) ([]state.Item, error) {
	if st == nil {
		return nil, errNoStateClient
	}
