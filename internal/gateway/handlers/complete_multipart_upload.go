...
func CompleteMultipartUpload(st Store, shards *storage.Client, ring *placement.Ring, cache *BucketCache, cfg Config) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := statePut(st, model.TableObjects, string(objectHash[:]), shardBuf.Bytes()); err != nil {
...
func getPartData(ctx context.Context, st Store, client *storage.Client, cfg Config, bucket, key, uploadID string, partNumber int) ([]byte, error) {
	data, err := stateGet(st, model.TableObjects, partShardKey(uploadID, partNumber))
	if err != nil {
