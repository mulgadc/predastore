...
func getUploadMetadata(st Store, uploadID string) (*model.UploadMetadata, error) {
	data, err := stateGet(st, model.TableMultipart, uploadID)
	if err != nil {
...
func requireUpload(st Store, bucket, key, uploadID string) error {
...
func getStoredParts(st Store, uploadID string) ([]model.PartMetadata, error) {
	items, err := stateScan(st, model.TableParts, multipartPartsPrefix(uploadID), 0)
	if err != nil {
		return nil, err
	}
...
func cleanupMultipartUpload(ctx context.Context, st Store, client *storage.Client, bucket, key, uploadID string, parts []model.CompletedPart) error {
	for _, part := range parts {
		...
		if data, err := stateGet(st, model.TableObjects, shardKey); err != nil {
