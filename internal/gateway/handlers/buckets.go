...
// BucketCache is the gateway's view of which buckets exist without a state
...
func lookupBucket(st Store, cache *BucketCache, bucket string) (*model.BucketMetadata, error) {
	...
	data, err := stateGet(st, model.TableBuckets, bucket)
	if err != nil {
...
func requireBucket(st Store, cache *BucketCache, bucket string) error {
...
func bucketExists(st Store, cache *BucketCache, bucket string) (exists bool, ownerID string, err error) {
	data, err := stateGet(st, model.TableBuckets, bucket)
	if err != nil {
		if !errors.Is(err, state.ErrNotFound) {
			return false, "", err
...
func getBucketMetadata(st Store, bucket string) (*model.BucketMetadata, error) {
	data, err := stateGet(st, model.TableBuckets, bucket)
	if err != nil {
		if errors.Is(err, state.ErrNotFound) {
			return nil, model.ErrNoSuchBucketError.WithResource(bucket)
...
//
// The gateway's auth middleware calls this for its cross-account ownership
// check, which is why bucket resolution lives with the handlers rather than
// inside one of them.
func ResolveBucketMetadata(st Store, cfg Config, bucket string) (*model.BucketMetadata, error) {
