...
func metaGet(mc MetaClient, table, key string) ([]byte, error) {
	if mc == nil {
		return nil, errNoMetaClient
	}
	return mc.Get(TableKey(table, key))
}

// metaPut writes one row of a table.
func metaPut(mc MetaClient, table, key string, value []byte) error {
	if mc == nil {
		return errNoMetaClient
	}
	return mc.Put(TableKey(table, key), value)
}

// metaDelete removes one row of a table.
func metaDelete(mc MetaClient, table, key string) error {
	if mc == nil {
		return errNoMetaClient
	}
	return mc.Delete(TableKey(table, key))
}
...
func metaScan(mc MetaClient, table, prefix string, limit int) ([]meta.Item, error) {
	if mc == nil {
		return nil, errNoMetaClient
	}
	items, err := mc.Scan(TableKey(table, prefix), limit)
	if err != nil {
		return nil, err
	}
	for i := range items {
