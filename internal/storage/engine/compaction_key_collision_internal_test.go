...
// collidingHash builds an object hash that starts with tombstonePrefix, so its
// shard key shares the tombstone namespace's prefix. Roughly one hash in 256
...
// TestCandidateSegmentsIgnoresShardKeysInTombstoneNamespace asserts that a live
// shard whose object hash begins with tombstonePrefix is not mistaken for a
// tombstone. Shard keys carry no namespace prefix, so a prefix scan for
...
// after it.
func TestCandidateSegmentsIgnoresShardKeysInTombstoneNamespace(t *testing.T) {
	st, dir := openTestStore(t, WithMaxSegSize(40*KiB))

	body := bytes.Repeat([]byte{0xcc}, 12*KiB)

	// Live shards only, every one colliding with the tombstone prefix. Nothing
	// is deleted, so a correct scan finds no tombstones and no candidates.
	const shards = 4
	for i := range shards {
		putShard(t, st, collidingHash(byte(i)), 0, body)
	}

	if got := tombstones(t, st); len(got) != 0 {
		t.Fatalf("no shard was deleted, so the tombstone namespace must be empty, got %v", got)
	}
	...
	if failed != 0 {
		t.Errorf("live shard keys were read as unreadable segments: failed=%d", failed)
	}
	if len(cands) != 0 {
		t.Errorf("live shard keys were selected as compaction candidates: %v", cands)
	}
	...
	for _, e := range entries {
		for i := range shards {
			...
			if e.Name() == filepath.Base(ghost) {
				t.Errorf("shard key fabricated segment %s from its own object hash", e.Name())
			}
		}
	}
}

// TestDeleteTombstonesSparesShardKeysInTombstoneNamespace pins the contract that
...
// stay on disk — a live shard lost silently.
//
...
func TestDeleteTombstonesSparesShardKeysInTombstoneNamespace(t *testing.T) {
	...
	// Target the exact segment number this shard's key would be misread as, so
	...
	// The shard must still resolve: its index row must have survived.
	r, err := st.Lookup(oh, 0)
	if err != nil {
		t.Fatalf("live shard lost after deleteTombstones swept its key: %v", err)
	}
	defer r.Close()
	...
	if !bytes.Equal(got, body) {
		t.Error("surviving shard corrupted")
	}
}
