...
func (b *Backend) ListBuckets(c *fiber.Ctx) error {
	Resp_ListBuckets := s3.ListBuckets{}

	for _, bucket := range b.Config.Buckets {

		Resp_ListBucket := s3.ListBucket{}
		Resp_ListBucket.Name = bucket.Name

		dir, err := os.Stat(bucket.Pathname)
...
// ListObjectsV2Handler handles ListObjectsV2 requests
func (b *Backend) ListObjectsV2Handler(bucket string, c *fiber.Ctx) error {
	...
	Resp_ListObjectsV2 := s3.ListObjectsV2{}
	Resp_ListObjectsV2_Contents := []s3.ListObjectsV2_Contents{}
	Resp_ListObjectsV2_Dir := []s3.ListObjectsV2_Dir{}
