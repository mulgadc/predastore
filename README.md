...
`DESTDIR` and `PREFIX` are honoured, so a package build can stage the same target into a buildroot. An existing `./bin/s3d` is installed as it stands rather than rebuilt, which is what lets the install run under `sudo` without `go` on root's `PATH`, so run `make build` first to pick up any source change.

The service runs as an unprivileged `predastore` user under a strict sandbox: `/etc` read-only, `/var/lib/predastore` the only writable path, no capabilities at all, and a syscall filter. Its host-local settings — which `[[host]]` it runs, where its data and keys live — come from `/etc/predastore/predastore.env`, which installs as `predastore.env.example` so an upgrade cannot overwrite an edited one. The shipped example config names those paths itself, which is what a single machine wants; a fleet distributing one identical TOML overrides them per host in the env file instead. The service logs at `warn`, rather than the `info` a hand-run `s3d` defaults to, on the grounds that a long-running service wants a quiet journal.
...
A freshly mounted drive is owned by root, and s3d refuses to start while any local node's directory is not writable by the user it runs as, naming each node and path it cannot write. Hand each drive's directory to the service user before starting it, e.g. `sudo chown predastore:predastore /var/lib/predastore/disk1 && sudo chmod 0700 /var/lib/predastore/disk1`. Under the systemd unit `/var/lib/predastore` is the only writable path, so mount the drives below it rather than elsewhere, or add their paths to `ReadWritePaths`.
